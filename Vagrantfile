...
Vagrant.configure(VAGRANT_API_VERSION) do |config|
  ...
  # vm configuration
  config.vm.box = ENV['ANSIBLE_UTIL_VAGRANT_BOXNAME'] || c['vm']['box']
  ...
  config.vm.define :ansibleutiltest do |d|
    ...
    d.vm.provision :ansible do |ansible|
      ...
      ansible.tags = ENV['ANSIBLE_UTIL_VAGRANT_ANSIBLE_TAGS']
      ansible.skip_tags = ENV['ANSIBLE_UTIL_VAGRANT_ANSIBLE_SKIP_TAGS']
      #   dynamic ansible-playbook configuration based on environment variables or user configuration
      ansible.verbose = ENV['ANSIBLE_UTIL_VAGRANT_ANSIBLE_VERBOSE'] || c['provisioner']['ansible']['verbose']

      # ansible-playbook raw arguments
      ansible.raw_arguments = []

      if ENV['ANSIBLE_UTIL_VAGRANT_ANSIBLE_CHECKMODE'] == '1'
        ansible.raw_arguments << '--check'
      end

      if ENV['ANSIBLE_UTIL_VAGRANT_ANSIBLE_DIFFMODE'] == '1' or c['provisioner']['ansible']['diff']
    ...
    d.vm.provider :virtualbox do |v|
      ...
      v.customize [ 'modifyvm', :id, '--name', 'ansibleutiltest', '--memory', '1024', '--cpus', '1' ]
    end
